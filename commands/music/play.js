...
module.exports = {
    ...
    async run(client, message, args) {
        ...
        let res = await client.player.search(args.join(' '), { requestedBy: message.member, searchEngine: QueryType.AUTO });
        if (!res || !res.tracks.length) return message.channel.send(`${message.author}, No results found! ❌`);
        let queue = await client.player.createQueue(message.guild, { metadata: message.channel });
        try {
