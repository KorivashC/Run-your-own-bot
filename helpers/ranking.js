...
module.exports = function (client, config) {
    ...
    const embedcolor = config.level_embed;
    client.on("message", async (message) => {
        ...
        if (message.content.startsWith(prefix)) {
            switch (command) {
                ...
                case `lb`:
                    ...
                    /////////////////////////////////
                case `setxpcounter`:
                if (!message.member.permissions.has("ADMINISTRATOR") || !message.member.permissions.has("MANAGE_GUILD")) return message.reply("You are not allowed to run this cmd!")
                    setxpcounter();
                break;
                    /////////////////////////////////
        ...
        function anti_double_messages() {
            ...
            if (oldmessage.toLowerCase() === message.content.toLowerCase().replace(/\s+/g, '')) return;
        ...
        function LEVELUP() {
            ...
            client.tokens.math(message.author.id, `+`, 50);

            /*if(newLevel == 5){
                let levelRole = message.guild.roles.cache.find(x => x.id === config.rolesIDs.levelFive);
                message.member.roles(levelRole);
            } else if(newLevel == 20){
                let levelRole = message.guild.roles.cache.find(x => x.id === config.rolesIDs.levelTwenty);
                message.member.roles(levelRole);
            } else if(newLevel == 35){
                let levelRole = message.guild.roles.cache.find(x => x.id === config.rolesIDs.levelThirtyfive);
                message.member.roles(levelRole);
            }*/
            const embed = new MessageEmbed()
                .setAuthor(`Ranking of:  ${message.author.tag}`, message.member.user.displayAvatarURL({dynamic: true}))
        ...
        function rank(the_rankuser) {
            try {
                ...
                if (!rankuser) return message.reply("Please enter a valid user!");
                if(rankuser.bot) return;
        ...
        async function leaderboard() {
            ...
            const lbembed = await message.channel.send({content: `**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds:[embeds[currentPage]]});
        ...
        function setxpcounter(){
            try {
                if (!args[0]) return message.reply("Please enter a valid user!");
                ...
                const key = `${message.guild.id}-${rankuser.id}`;
                databasing(rankuser);
                if (!args[1]) return message.reply("Please enter a vaild number! Usage: `setxpcounter @USER 2`");
        ...
        function setglobalxpcounter(){
            try {
                if (!args[0]) return message.reply("Please enter a vaild number! Usage: `setglobalxpcounter 2`");
        ...
        function addpoints(amount) {
            try {
                ...
                const neededPoints = client.points.get(key, `neededpoints`);
                let leftpoints = neededPoints - curPoints;
                if (!args[1] && !amount) return message.reply("Please enter a vaild number! Usage: `addpoints @USER 100`");
                ...
                function addingpoints(toaddpoints, leftpoints) {
                    if (toaddpoints >= leftpoints) {
                        ...
                        if (toaddpoints - leftpoints < newneededPoints) message.channel.send( {embeds: [embed]} );
                        addingpoints(toaddpoints - leftpoints, newneededPoints); //Ofc there is still points left to add so... lets do it!
                    } else {
        ...
        function setpoints() {
            try {
                ...
                if (!args[1]) return message.reply("Please enter a valid number! Usage: `addpoints @USER 100`");
                ...
                function addingpoints(toaddpoints, neededPoints) {
                    if (toaddpoints >= neededPoints) {
                        ...
                        const embed = new MessageEmbed()
                            ...
                            .setDescription(`You've leveled up to Level: **\`${newLevel}\`**! (Points: \`${newPoints}\` / \`${newneededPoints}\`) `)
                            .setColor(embedcolor);
                        message.channel.send(rankuser, {embeds: [embed]} );
        ...
        function removepoints(amount) {
            try {
                ...
                if (!args[1] && !amount) return message.reply("Please enter a valid number! Usage: `addpoints @USER 100`");
                if (!amount) amount = Number(args[1]);
